    ...
    Effect.tap((result) =>
      Result.isFailure(result)
        ? Effect.logWarning(
            ...
            // A probe that exceeded CAPABILITIES_PROBE_TIMEOUT_MS surfaces here
            // as a TimeoutException: upstream moved from timeoutOption to
            // Effect.timeout, so a timeout is a failure rather than Option.none.
            { error: String(result.failure), timeoutMs: CAPABILITIES_PROBE_TIMEOUT_MS },
          )
        : Effect.void,
    ),
